...
final class core_course_course_test extends \advanced_testcase {
    ...
    public function test_program_course_completions_purged(): void {
        ...
        if (!get_config('enrol_programs', 'version')) {
            $this->markTestSkipped('enrol_programs not installed');
        }
        ...
        \enrol_programs\local\course_reset::purge_completions($user1, $program1->id);
        ...
        \enrol_programs\local\course_reset::purge_completions($user2, $program1->id);
        $this->assertCount(1, $DB->get_records('customfield_training_completions', []));
        $this->assertCount(1, $DB->get_records('customfield_training_completions',
            ['fieldid' => $field1->get('id'), 'instanceid' => $course3->id, 'userid' => $user1->id]));
    }
